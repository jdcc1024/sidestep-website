...
## Vertical Slice
This issue touches:
- [ ] Database: `jerseyRunResponses` table — write via Convex mutation; read `jerseyRuns` to render form
- [ ] API: Convex query `getJerseyRun` (public); mutation `submitJerseyRunResponse`
- [ ] Frontend: Public (no-auth) fan submission page at `/run/<id>`; closed state when deadline passed or status is "closed"
- [ ] Tests: Submission creates response record; empty name/number triggers warning; closed form shows correct state; fixed vs open mode renders correctly
...
## Acceptance Criteria
- [ ] Public route `/run/[id]` — accessible without authentication
- [ ] Form fields: respondent's real name (required), email (required, for captain's reference), jersey selection (from run's size options), name on jersey (with "leave blank for no name" note — triggers a warning modal before submit if blank), number on jersey (same blank-warning behavior), answers to any custom questions defined by the captain
- [ ] When `namesMode === "fixed"`: show a dropdown/select of the captain's fixed roster; respondent picks their entry
- [ ] When `namesMode === "open"`: show free-text fields for name and number
- [ ] Empty name or number: allow submission but display a warning ("Are you sure you want to leave your jersey name/number blank?") with Confirm / Go Back options
- [ ] On submission, create a `jerseyRunResponse` record in Convex
- [ ] Show a confirmation message after successful submission: "You're in! Your captain will be in touch."
- [ ] If jersey run `status === "closed"` or deadline has passed: show a "This jersey run has closed" message; hide the form
- [ ] All tests pass
